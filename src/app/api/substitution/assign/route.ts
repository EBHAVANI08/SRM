...
Return ONLY the JSON, no markdown.`
      ...
      await auditLog({
        ...
        action: 'ASSIGN',
        module: 'SUBSTITUTION',
        description: `Substitute ${bestMatch.staffName} assigned to cover ${substitution.originalTeacherId}'s period ${substitution.period} class ${substitution.classId} on ${new Date(substitution.date).toDateString()}. AI match score: ${Math.round(aiMatchScore * 100)}%.`,
        metadata: {
      ...
      await alertNotify({
        ...
        message: `${bestMatch.staffName} has been assigned to cover a ${substitution.subject} period ${substitution.period} class on ${new Date(substitution.date).toDateString()}. AI match score: ${Math.round(aiMatchScore * 100)}%.`,
