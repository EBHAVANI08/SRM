    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            {/* Header */}
            <div className="relative px-5 py-4 bg-gradient-to-r from-violet-600 via-fuchsia-600 to-orange-500 text-white overflow-hidden">
              ...
              <div className="relative flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="relative">
                    ...
                    <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-400 rounded-full border-2 border-violet-600 animate-pulse" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-base">LearnX AI</span>
                      <Badge className="bg-white/20 text-white border-0 text-[9px] h-4 px-1.5">RAG</Badge>
                    </div>
                    <div className="text-[11px] text-white/80 flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-green-300" />
                      Online · GPT-4 Class · Context-aware
                    </div>
                  </div>
                </div>
                <button
              ...
              <div className="space-y-4">
                ...
                {loading && (
                  <div className="flex gap-3">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-orange-500 text-white flex items-center justify-center">
                      <Bot className="w-4 h-4" />
                    </div>
                    <div className="bg-slate-100 rounded-2xl rounded-tl-sm px-4 py-3 flex items-center gap-2">
                      <Loader2 className="w-3.5 h-3.5 animate-spin text-violet-500" />
                      <span className="text-xs text-slate-600">Searching knowledge base & reasoning...</span>
                    </div>
                  </div>
                )}
                    ...
                    <div className="space-y-1.5">
                      {followUps.slice(0, 4).map((q, i) => (
                        <button
                          key={i}
                          onClick={() => send(q)}
                          className="group w-full flex items-center justify-between gap-2 p-2.5 rounded-xl border border-slate-200 hover:border-violet-300 hover:bg-violet-50 transition-all text-left"
                        >
                          <span className="text-xs text-slate-700 group-hover:text-violet-700">{q}</span>
                          <ArrowRight className="w-3 h-3 text-slate-400 group-hover:text-violet-600 group-hover:translate-x-0.5 transition-all flex-shrink-0" />
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              >
                <div className="flex-1 relative">
                  <textarea
                    ...
                    rows={1}
                    className="w-full px-3.5 py-2.5 text-sm rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-violet-300 focus:ring-2 focus:ring-violet-100 resize-none max-h-32 transition-all"
                  />
                </div>
                <Button
                  type="submit"
                  ...
                  size="icon"
                  className="h-10 w-10 rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700"
                >
